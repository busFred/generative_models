...
def _standard_truncnorm_sample(lower_bound: torch.Tensor,
    ...
    while not done.all():
        proposed_x = lower_bound + torch.rand(sample_shape) * (upper_bound -
                                                               lower_bound)
...
class TruncatedNormal(TruncatedDistribution):
    ...
    def __init__(self,
        ...
        self.loc = loc
        self.scale = scale
        super().__init__(Normal(loc, scale), lower_bound, upper_bound, *args,
                         **kwargs)
        # TODO: default values for lower and upper bounds

    def rsample(self, sample_shape=torch.Size()):
        eps = _standard_truncnorm_sample(
            (self.lower_bound - self.base_dist.mean) / self.base_dist.stddev,
    ...
    @property
    def mean(self):
        return (self.base_dist.mean + self.base_dist.stddev *
    ...
    def variance(self):
        return self.base_dist.variance * (
            1 + (self.lower_bound * self.base_dist.log_prob(
                self.lower_bound).exp() - self.upper_bound *
                 self.base_dist.log_prob(self.upper_bound).exp()) / self.Z -
            (self.base_dist.log_prob(self.lower_bound).exp() -
             self.base_dist.log_prob(self.upper_bound).exp())**2 / self.Z**2)
    ...
    @property
    def entropy(self):
        return (0.5 * math.log(2 * math.pi) + self.stddev.log() +
                self.Z.log() + 0.5 *
                (self.lower_bound * self.base_dist.log_prob(
                    self.lower_bound).exp() - self.upper_bound *
                 self.base_dist.log_prob(self.upper_bound).exp()) / self.Z)
