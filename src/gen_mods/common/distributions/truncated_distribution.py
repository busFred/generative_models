...
class TruncatedDistribution(Distribution):
    ...
    def __init__(self,
        ...
        self.lower_bound, self.upper_bound = broadcast_all(
            lower_bound, upper_bound)
        cdf_low, cdf_high = self.base_dist.cdf(
            self.lower_bound), self.base_dist.cdf(self.upper_bound)
    ...
    def rsample(self, sample_shape=torch.Size()):
        ...
        u = param.new(shape).uniform_(
            torch.finfo(param).tiny, 1 - torch.finfo(param).tiny)
        return self.base_dist.icdf(
            self.base_dist.cdf(self.lower_bound) + u * self.Z)
    ...
    def log_prob(self, value):
        ...
        log_prob[(value < self.lower_bound) |
                 (value > self.upper_bound)] = -float('inf')
    ...
    def cdf(self, value):
        ...
        return (self.base_dist.cdf(value) -
                self.base_dist.cdf(self.lower_bound)) / self.Z
